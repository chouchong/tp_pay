...
class AliPay
{
    ...
    private $keys = 'twcndfk8nw11psavd2k1kzazchu0t1s4';
    ...
    // wap
    public function wapalipay($data){
        header("Content-type:text/html;charset=utf-8");
        $param = array(
            'service'=>'alipay.wap.create.direct.pay.by.user',
            // 'service'=>'create_direct_pay_by_user',
    ...
    function md5Verify($prestr, $sign, $key) {
        $prestr = $prestr . $key;
        $mysgin = md5($prestr);
        Log::write('订单1' . $mysgin . '支付失败');
        Log::write('订单2' . $sign . '支付失败');
        if($mysgin == $sign) {
            return true;
        }
        else {
